...
export class CartComponent implements OnInit {
  ...
  clearCart(): void {
    ...
    this.loadCart();
    this.orderPlaced = false; // Reset orderPlaced when cart is cleared
  }

  handleCheckOut(): void {
    if (!this.apiService.isAuthenticated()) {  //login page redirection
    ...
    const orderItems = this.cart.map(item => ({  //item with quantity
      productId: item.id,
    ...
    this.apiService.createOrder(orderRequest).subscribe({
      next: (response) => {
        ...
        if (response.status === 200) {
          this.cartService.clearCart();
          this.loadCart();
          this.orderPlaced = true; // Set orderPlaced to true when order is successfully placed
        }
      },
      error: (error) => {
        console.log(error);
